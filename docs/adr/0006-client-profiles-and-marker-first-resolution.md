...
Named profiles are full style-guide snapshots in `~/.diagram-design/profiles/`, with one metadata header naming the active profile. An optional project-root `.diagram-design` file marker selects a validated slug and reads that home profile directly. A directory marker uses `.diagram-design/current-profile`; for non-default slugs it first resolves a committed `.diagram-design/profiles/<slug>.md`, then falls back to the home library. `default` is reserved for the protected shipped profile and always resolves from the home library. Repository-local profiles remain untrusted data, pass the same structural check, and are maintained manually rather than by profile verbs. Only an explicit markerless load copies a profile into the installed working file. Loads check the current semantic-role and typography schema and backfill missing rows from shipped defaults.
