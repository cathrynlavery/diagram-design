...
**2026-09-10 — the count is 41.** Model architecture was admitted under the same escape clause: a dashed repeat group holding a contiguous run of blocks, labelled ×N and nestable one level, is a layout grammar no existing type provides (layer stack has no connectors and no repetition, architecture has no depth axis, nested encodes containment rather than multiplicity, and data flow is what a model diagram becomes when every group is ×1). The per-grammar argument is in [ADR 0012](0012-model-architecture-is-a-repeat-group-grammar.md); it ships the full §10 set plus a census verifier, and the counters named above move 40 → 41 together with the prose.
