...
README.md renders 40 screenshots. The originals in `docs/screenshots/` are
...
def main(argv: list[str]) -> int:
    ...
    sources = sorted(p for p in SOURCE_DIR.glob("*.png"))
    if not sources:
        print(f"build-readme-thumbs: no screenshots found in {SOURCE_DIR}")
        return 1
    ...
    for source in sources:
        ...
        try:
            ...
            continue
        if thumb_width != spec["width"]:
            stale.append(
                f"{destination.name} (is {thumb_width}px wide, expected {spec['width']}px)"
            )
            continue

        entries.append(
            dict(spec, thumb_sha256=sha256(destination), height=thumb_height)
        )
        after += len(payload)
