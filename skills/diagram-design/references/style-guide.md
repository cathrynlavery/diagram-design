...
Simplified Chinese takes the same three rules with the Simplified stack (`'Noto Sans SC'`, `'PingFang SC'`, `'Microsoft YaHei'`). Both Noto SC faces ship in the font link above, so Simplified labels resolve as web fonts first and render identically across machines. Offline viewers fall back to `'Microsoft YaHei'`. 
### Cyrillic labels
