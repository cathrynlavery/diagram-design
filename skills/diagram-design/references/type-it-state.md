...
eyebrow:  "STATIN · Jamaica · Before D4N"
...
zones:                       # 2..4 zones, ordered along the orientation axis
  ...
  - name: "DISSEMINATION"
    components:
      - { id: datazoa,         name: "DataZoa",           sub: "manual bottleneck",     icon: cloud,    kind: focal }
      - { id: statin-website,  name: "STATIN Website",    sub: "public · static pages", icon: internet }
...
connectors:                   # ordered list; each links two component ids
  ...
  - { from: analyst-mach,     to: datazoa,        label: "EXCEL",   icon: excel, style: accent }
  - { from: datazoa,          to: statin-website, label: "WEB",                  style: neutral }
  - { from: statin-website,   to: ministry,       label: "CSV DL",  icon: csv,   style: link, dashed: true }
...
- `zones[i].name` — uppercase short label (≤ 14 chars). Rendered at the top-left of the zone box in JetBrains Mono 7 letter-spacing 0.14em, on a paper-masked break in the zone border.
- `components[i][k].id` — globally unique slug; referenced by `connectors[].from/to`.
- `components[i][k].name` — Montserrat 12 weight 600 (the human-readable label).
- `components[i][k].sub` — Roboto 10 muted (the technical sub-label; up to 2 lines via auto-wrap when component height grows to 72).
...
- `connectors[k].label` — uppercase short text (≤ 8 chars). JetBrains Mono 9 weight 600.
...
<rect x="zone_x(i)" y="zone_y" width="zone_w(i)" height="zone_h"
      fill="rgba(11,31,56,0.02)" stroke="rgba(11,31,56,0.10)" stroke-width="0.8" rx="8"/>
<!-- paper-masked break for the zone label -->
<rect x="zone_x(i)+20" y="zone_y-8" width="{label_w}" height="16" fill="{paper}"/>
<text x="zone_x(i)+24" y="zone_y+4" fill="rgba(11,31,56,0.40)"
      font-family="'JetBrains Mono',monospace" font-size="7" letter-spacing="0.14em">{name}</text>
```
...
| --- | --- | --- | --- | --- | --- | --- |
| `standard` | `#FFFFFF` | `#0B1F38` | 1 | — | `#0B1F38` | `#5A6B82` |
| `focal` | `rgba(230,53,88,0.07)` | `#E63558` | 1.4 | — | `#0B1F38` | `#E63558` (line 1, italic) + `#5A6B82` (line 2) |
| `external` | `#FFFFFF` | `#5A6B82` | 1 | `4,3` | `#0B1F38` | `#5A6B82` |
...
<rect x="left_pad" y="footer_y(k)" width="viewBox_w - 2*left_pad" height="footer_bar_h"
      fill="rgba(11,31,56,0.03)" stroke="rgba(11,31,56,0.18)" stroke-width="0.8" rx="8"/>
<g transform="translate(left_pad + 20, footer_y(k) + (footer_bar_h - 24)/2)" color="#0B1F38">
  <use href="#icon-{name}"/>
</g>
<text x="left_pad + 56" y="footer_y(k) + 24" font-family="Montserrat" font-size="14" font-weight="600" fill="#0B1F38">{name}</text>
<text x="left_pad + 56" y="footer_y(k) + 40" font-family="Roboto"    font-size="12"               fill="#5A6B82">{sub}</text>
```
...
| --- | --- | --- | --- |
| `neutral` | `#5A6B82` | 1.0 | `url(#arrow)` |
| `link` | `#167C7C` | 1.2 | `url(#arrow-link)` |
| `accent` | `#E63558` | 1.4 | `url(#arrow-accent)` |
...
```svg
<defs>
  <marker id="arrow"        markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#5A6B82"/></marker>
  <marker id="arrow-link"   markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#167C7C"/></marker>
  <marker id="arrow-accent" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#E63558"/></marker>
</defs>
```
...
- Mask `fill` is paper (light `#F4F6FA`, dark `#0B1F38`). The mask is kept as a safety pad — even though the label no longer sits on the line, it can graze zone backgrounds and component fills, and the paper mask preserves contrast.
...
- Auto-styling: accent stroke 1.4, accent-tinted fill 7 %, ink-bold name, accent-italic line-1 sub-label (Roboto italic).
...
| link | `#167C7C` | `#3DA8A8` |
| zone background | `rgba(11,31,56,0.02)` | `rgba(244,246,250,0.04)` |
| zone border | `rgba(11,31,56,0.10)` | `rgba(244,246,250,0.14)` |
| standard component fill | `#FFFFFF` | `rgba(244,246,250,0.04)` |
| standard component stroke | `#0B1F38` | `rgba(244,246,250,0.32)` |
| focal fill | `rgba(230,53,88,0.07)` | `rgba(238,85,112,0.12)` |
| focal stroke | `#E63558` | `#EE5570` |
| external stroke | `#5A6B82` (dashed) | `#9FB0C9` (dashed) |
| footer fill | `rgba(11,31,56,0.03)` | `rgba(244,246,250,0.05)` |
| footer stroke | `rgba(11,31,56,0.18)` | `rgba(244,246,250,0.20)` |
| label mask fill | `#F4F6FA` (paper) | `#0B1F38` (paper) |
...
10. JetBrains Mono for connector labels, eyebrow, zone labels. Montserrat for title + component names. Roboto for subtitle + sub-labels.
11. Markers `#arrow` / `#arrow-link` / `#arrow-accent` defined once in `<defs>`; no inline marker definitions.
12. Dark variant: paper/ink inverted, accent → `#EE5570`, link → `#3DA8A8`, custom colors lightened ~15 %.

---

## 8. Anti-patterns

- **Diagonal arrows.** The STATIN replication has one (analyst → DataZoa). The new type forbids it — always rounded right-angle Q-bezier.
...
- **Tiny text badges as icons.** The source uses 7-px mono `DB` / `APP` / `EXT`; this type uses real 24-px catalog icons. Text badges are only acceptable as the label text, not as the component "icon."
...
- `assets/example-it-state.html` — minimal light (STATIN canonical: 3 zones, 9 components, 8 connectors, 0 footer bars, SQL Server tinted olive). Gallery default.
...
- `n_zones = 3`, components per zone `= [3, 3, 3]`, custom color count = 1 (SQL Server), focal count = 2 (Shared Drive, DataZoa), external count = 2 (Civil Registry, Ministry Partners).
...
- DataZoa (focal) at zone 3, row 0: `x = 704, y = 80, w = 208, h = 60` ✓
...
- Connectors 4, 5 (within zone 2) and 7, 8 (within zone 3) are simple vertical `<line>` elements. Cross-zone connectors take rule-compliant routes (see SKILL.md §6 rules 4 & 5):
  ...
  - **C6** (Analyst Machines → DataZoa) cannot use a direct H+Q+V into DataZoa's left edge — Analyst Machines and DataZoa are in different rows, and the direct horizontal at `y = 268` would cross STATIN Website. It detours through the zone gap and **over** DataZoa: `H 654 Q 662,268 662,260 V 72 Q 662,64 670,64 H 800 Q 808,64 808,72 V 80` — vertical at `x = 662` (in zone gap), horizontal at `y = 64` (above DataZoa top), then down into DataZoa's top center. The path enters DataZoa from **above** going down, so the arrow body lives above the box (visible) and only the 1-px tip enters the box.
