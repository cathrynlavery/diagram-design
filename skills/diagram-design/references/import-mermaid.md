...
Grammar details the digest preserves. A `timeline` header may carry `LR` or `TD` and the declared direction is reported. Repeated gantt `excludes` / `includes` lines accumulate the way Mermaid concatenates them rather than overwriting. A quadrant point's `:::class` attachment is styling, so it is counted as discarded and kept out of the label. A mindmap topic whose Markdown string spans several lines is rejoined into one topic, since its continuation lines would otherwise read as deeper topics. A mindmap shape written without an identifier (`((Root))`) keeps its shape and inner label, and a mindmap declaring a second root exits 2. A timeline event is separated by a colon *followed by whitespace*, matching `timeline.jison`, so a `://` inside event text survives; `#` opens a comment there. `accTitle:` / `accDescr:` describe the rendered figure rather than its content, so they are dropped like frontmatter in every grammar, and a trailing `%%` comment is stripped before the new parsers read a line.
...
| `mindmap has a second root at line N` | Mermaid's mindmap has one root; a second one means the indentation is wrong, not that there are two trees. |
