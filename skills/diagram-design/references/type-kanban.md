...
- **Header band per column:** column name in Geist sans 12px weight 600, anchored left. A right-anchored **WIP chip** — a rectangular tag (`rx=2`, **never** a pill) holding `n/limit` in Geist Mono 8px. A hairline `rule` separator sits directly under the header band, full column width.
...
- **Columns without WIP limits.** The limit is half the point of the type — a column with no stated limit can't show that it's over budget.
