...
subtitle: "LFS quarterly survey through Raw → Anonymized → Staging → Aggregated → Archive"
...
example_label: "LFS example"          # bottom field label (varies per domain)
...
title_text            at (tier_cx(i), 106)                # font 13, weight 700 sans, ink
bucket_text           at (tier_cx(i), 144)                # font 9 mono, muted (accent on focal tier)
...
Field label font: Montserrat 11, weight 600, ink. Field value font: JetBrains Mono 9, muted. Bucket and field values can be retinted by `color` override (§4).
...
|---|---|---|---|---|---|
| `outer` | `#FFFFFF` | `#5A6B82` 1.0 solid | `rgba(90,107,130,0.10)` | `muted` | `muted` |
| `default` | `#FFFFFF` | `#0B1F38` 1.0 solid | `rgba(11,31,56,0.06)` | `muted` | `muted` |
| `focal` | `rgba(230,53,88,0.07)` | `#E63558` 1.6 solid | `rgba(230,53,88,0.14)` | `accent` | `accent` |
| `cold` | `#E4E9F0` (fog) | `#5A6B82` 1.0 dashed `5,3` | `rgba(90,107,130,0.18)` | `muted` | `muted` |
...
|---|---|---|---|---|
| `normal` | `#5A6B82` (muted) | 1.4 | — | `arrow` |
| `focal` | `#E63558` (accent) | 1.6 | — | `arrow-accent` |
| `lifecycle` | `#5A6B82` (muted) | 1.4 | `4,3` | `arrow` |
...
- Mono `font-size=10`, `letter-spacing=0.08em`, uppercase. Color matches the arrow stroke.
...
Per-card content:
- Container rect: white fill, ink-stroke `rgba(11,31,56,0.20)` width 1, `rx=6`.
- Tag chip: rect at `(path_x + 8, path_y + 6)`, `h=12 rx=2`, fill transparent, stroke `rgba(11,31,56,0.30)` width 0.8. Tag text centered inside, Mono 7, letter-spacing 0.08em, ink.
- Title at `(path_x + 80, path_y + 30)`: Montserrat 11, weight 600, ink.
- Sub at `(path_x + 80, path_y + 46)`: Mono 9, muted.
...
| Fog (cold tier fill) | `#E4E9F0` | `rgba(244,246,250,0.06)` |
| White (default card fill) | `#FFFFFF` | `rgba(244,246,250,0.04)` |
| Card stroke ink (default style) | `#0B1F38` | `rgba(244,246,250,0.30)` |
| Header band ink-tint | `rgba(11,31,56,0.06)` | `rgba(244,246,250,0.08)` |
| Header band muted-tint | `rgba(90,107,130,0.10)` | `rgba(138,153,174,0.16)` |
| Header band cold-tint | `rgba(90,107,130,0.18)` | `rgba(138,153,174,0.24)` |
| Header band accent-tint | `rgba(230,53,88,0.14)` | `rgba(238,85,112,0.20)` |
...
- **Missing `example_label` content** — every tier should show a concrete example payload (LFS rows, customer records, claims, …). Without it the diagram becomes abstract and stops earning its space.
- **Promotion arrow label longer than the tier-gap label mask** — keep labels to ≤ 14 chars and uppercase mono. Long verbs ("CALCULATE & SUMMARIZE") break the rhythm; shorten to "AGGREGATE" or split into two diagrams.

---

## 9. Examples

- `assets/example-medallion.html` — minimal light (STATIN LFS: 5 tiers, 2 path cards, Aggregated focal). Gallery default.
